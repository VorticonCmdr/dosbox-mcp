...
def register(server, client, add_tool, feature=None, annotate=None):
    ...
    add_tool(
        ...
        description=(
            ...
            "immediately rather than accepting a breakpoint that would "
            "never fire). Three ceilings decide whether a quiet "
            "watchpoint means anything: it covers exactly one byte, so "
            "a word needs one on each half and a store that only "
            "changes the high byte never fires one armed at the low "
